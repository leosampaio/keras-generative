...
class TOPGANwithContrastiveAEfromBEGAN(BaseModel):
    name = 'topgan-con-ae-began'
    loss_names = ['g_loss', 'd_loss', 'd_triplet',
                  'g_triplet', 'ae_loss', 'contrastive_ae_n']
    ...
    def __init__(self,
                 ...
                 triplet_margin=1.,
                 n_filters_factor=32,
                 share_decoder_and_generator=False,
                 **kwargs):
        ...
        self.share_decoder_and_generator = share_decoder_and_generator
        if share_decoder_and_generator:
            self.z_dims = self.embedding_size
    ...
    def train_on_batch(self, x_data, y_batch=None, compute_grad_norms=False):
        ...
        y_pos, y_neg = smooth_binary_labels(batchsize // 2, self.label_smoothing, one_sided_smoothing=False)
        y = np.stack((y_neg, y_pos), axis=1)

        # get real latent variables distribution
        z_latent_dis = np.random.normal(size=(batchsize // 2, self.z_dims))
        ...
        x_permutation = np.array(np.random.permutation(batchsize // 2), dtype='int64')
        input_data = [x_data[:len(x_data) // 2], noise[:len(x_data) // 2], x_permutation, z_latent_dis]
        label_data = [y, y, x_data[:len(x_data) // 2], x_data[len(x_data) // 2:]]

        # train both networks
        ld = {}  # loss dictionary
        _, ld['d_loss'], ld['d_triplet'], ld['ae_loss'], ld['contrastive_ae_n'] = self.dis_trainer.train_on_batch(input_data, label_data)
        _, ld['g_loss'], ld['g_triplet'], _, _ = self.gen_trainer.train_on_batch(input_data, label_data)
    ...
    def build_trainer(self):
        ...
        output = [concatenated_dis, concatenated_triplet, x_reconstructed, x_reconstructed]
        return Model(input, output, name='topgan')

    def build_model(self):

        self.f_D = self.build_D()
        if self.share_decoder_and_generator:
            self.f_Gx = self.decoder
        else:
        ...
        self.dis_trainer.compile(optimizer=self.optimizers["opt_d"],
                                 loss=[loss_d, triplet_d_loss, 'mse', 'mse'],
                                 loss_weights=[self.losses['d_loss'].backend, self.losses['d_triplet'].backend, self.losses['ae_loss'].backend, -self.losses['contrastive_ae_n'].backend])
        ...
        self.gen_trainer.compile(optimizer=self.optimizers["opt_g"],
                                 loss=[loss_g, triplet_g_loss, 'mse', 'mse'],
                                 loss_weights=[self.losses['g_loss'].backend, self.losses['g_triplet'].backend, self.losses['ae_loss'].backend, -self.losses['contrastive_ae_n'].backend])
...
class TOPGANwithContrastiveAESmall(TOPGANwithContrastiveAEfromBEGAN):
    name = 'topgan-con-ae-small'
...
class TOPGANwithContrastiveAEfromDCGAN(TOPGANwithContrastiveAEfromBEGAN):
    name = 'topgan-con-ae-dcgan'
...
class TOPGANwithContrastiveAEwithResNet(TOPGANwithContrastiveAEfromBEGAN):
    name = 'topgan-con-ae-res'
