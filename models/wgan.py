...
def discriminator_lossfun(y_true, y_pred):
    """
    y_pred[:,0]: p, prediction for D(Gx(z))
    y_pred[:,1]: q, prediction for D(x)
    ...
    q_error = -K.mean(p*p_true)
    p_error = -K.mean(q*q_true)
...
def grad_penalty_loss(y_true, y_pred):
    """
    considers y_pred to be the precalculated Gradient Norm
    """

    return K.mean(K.square(y_pred - 1.))


class ImprovedWGAN(BaseModel, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.svm = LinearSVC()
        self.svgan_type = 'batch_svm'  # or 'batch_svm'
